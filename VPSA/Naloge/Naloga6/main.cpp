...
int mandelbrotGPU(PGMData *IMG)
{
	...
	cl_mem mem_obj = clCreateBuffer(context, CL_MEM_WRITE_ONLY, imageSize * sizeof(int), NULL, &ret);
	...
	ret |= clSetKernelArg(kernel, 1, sizeof(cl_mem), (void *)&width);
	ret |= clSetKernelArg(kernel, 2, sizeof(cl_mem), (void *)&height);
	...
	ret = clEnqueueReadBuffer(command_queue, mem_obj, CL_TRUE, 0, imageSize * sizeof(int), IMG->image, 0, NULL, NULL);
